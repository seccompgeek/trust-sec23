...
do
    ...
    do
	    ...
	    eval $SVF $SVFARGS $bc_file &> output.txt
            ...
            cat output.txt | grep Cloned
            cat output.txt | grep "Total Unsafe Objects: "
            echo "SVF successfully run"
            echo "Running OPT on $bc_file"
            $LLVM_OPT -trust-enable-mpk -mpk-isolation -mpk-isolation-gates $bc_file -o $bc_file &> output.txt
            if [[ $? -ne 0 ]]; then
                cat output.txt
                exit -1
            fi 
            ...
            $LLVM_LLC $bc_file -trust-enable-mpk -relocation-model=pic -filetype=obj &> output.txt
